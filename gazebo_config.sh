...
if [ -n "$GAZEBO_RESOURCE_PATH" ];then  # the GAZEBO_RESOURCE_PATH is not empty showing the gazebo has been configured
    echo "export GAZEBO_RESOURCE_PATH=$GAZEBO_RESOURCE_PATH:${current_dir}" >> ~/."${SHELL##*/}"rc
    GAZEBO_RESOURCE_PATH=$GAZEBO_RESOURCE_PATH:${current_dir} && export GAZEBO_RESOURCE_PATH
else
